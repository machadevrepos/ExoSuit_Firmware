...
>   see the AGENTS.md invariant). Awaiting hardware verification. P2/P3 list in §5.
...
- GATT event-mask arg per char (vendor values from `ble_defs.h:390-393`: ATTRIBUTE_WRITE 0x01, WRITE_REQ_AND_WAIT 0x02, READ_REQ_AND_WAIT 0x04, **NOTIFICATION_COMPLETION 0x08**):
  ...
  - **Node PipeDataTx = 0x0F** since 2026-09-13 (`feature/BLE_speed_Optimization`; the completion counter is filtered to the PipeDataTx value handle at the VS-event dispatch). **Node other chars = 0x07**. Length types: both PipeDataTx are `CHAR_VALUE_LEN_VARIABLE` (the Master compiles its CONSTANT token to VARIABLE at `custom_stm.cpp:63-64`; do not switch the Node to fixed-length — CPU2 behavior for shorter-than-declared updates is undocumented). Both size 244. Both are edits outside USER CODE regions.
...
| `0xB5` | transfer tuning wire — fast-interval byte in 1.25 ms units, supported set `{24,16,12,9,6}` = 30 / **20 (firmware-owned bulk)** / 15 / 11.25 / 7.5 ms, default 12 (15 ms), `kBulkFastInterval = 16` (20 ms, since 2026-09-13; was 24/30 ms) (`record_transfer_tuning.h:20-45`) |
...
> **APPLIED 2026-09-13 on `feature/BLE_speed_Optimization`** (awaiting hardware
