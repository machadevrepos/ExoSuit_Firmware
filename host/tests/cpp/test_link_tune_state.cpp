...
void test_bulk_ce_hint_survives_fallback_ladder()
{
  Model model;
  uint32_t now = 100U;
  EXPECT_TRUE(model.connect(0U, 0x0040U, now));
  ...
  now += Model::kCommissioningStartDelayMs;
  Model::Request request = model.issue_next(now);
  EXPECT_TRUE(request.valid() && request.procedure == Procedure::Dle);
  EXPECT_TRUE(model.on_request_accepted(request, now));
  EXPECT_TRUE(model.on_dle_complete(request.handle, generation, 251U, 251U, now));
  request = model.issue_next(now);
  EXPECT_TRUE(request.valid() && request.procedure == Procedure::Phy);
  EXPECT_TRUE(model.on_request_accepted(request, now));
  EXPECT_TRUE(model.on_phy_complete(request.handle, generation,
                                    Model::kStatusSuccess, 2U, 2U, now));
  EXPECT_TRUE(model.telemetry(0U).state == State::Ready);

  /* Bulk profile: the 30 ms CE hint must survive every fallback level. The
   * exact 20 ms request (0x0010) is grantable at the observed 40 ms anchor,
   * and each scheduler rejection (0x84) must NOT demote the CE hint to the
   * live 5 ms budget - that demotion was the measured 1-notification-per-CE
   * bulk pipeline. */
  EXPECT_TRUE(model.begin_fast_preparation(0U, generation, TuningWire::kBulkFastInterval));
  EXPECT_TRUE(TuningWire::sanitize_fast_interval(TuningWire::kBulkFastInterval) ==
              TuningWire::kBulkFastInterval);
  EXPECT_TRUE(TuningWire::kBulkFastInterval == 16U);

  request = model.issue_next(now);
  EXPECT_TRUE(request.valid() && request.procedure == Procedure::Interval &&
              request.interval == Model::Interval::Fast);
  EXPECT_TRUE(request.interval_min == TuningWire::kBulkFastInterval &&
              request.interval_max == TuningWire::kBulkFastInterval);
  EXPECT_TRUE(request.min_ce_length == Model::kBulkMinCeLength &&
              request.max_ce_length == Model::kBulkMaxCeLength);
  EXPECT_TRUE(model.on_request_status(request, Model::kStatusNewIntervalFailed, now));

  now += Model::kRetryDelayMs;
  request = model.issue_next(now);
  EXPECT_TRUE(request.valid());
  EXPECT_TRUE(request.interval_min == TuningWire::kBulkFastInterval &&
              request.interval_max == TuningWire::kBulkFastInterval + Model::kFastIntervalFallbackSpan);
  EXPECT_TRUE(request.max_ce_length == Model::kBulkMaxCeLength);
  EXPECT_TRUE(model.on_request_status(request, Model::kStatusIntervalTooLarge, now));

  now += Model::kRetryDelayMs;
  request = model.issue_next(now);
  EXPECT_TRUE(request.valid());
  EXPECT_TRUE(request.interval_min == Model::kSubMultipleIntervalMin &&
              request.interval_max == Model::kSubMultipleIntervalMax);
  EXPECT_TRUE(request.max_ce_length == Model::kBulkMaxCeLength);
  EXPECT_TRUE(model.on_request_status(request, Model::kStatusLengthFailed, now));

  now += Model::kRetryDelayMs;
  request = model.issue_next(now);
  EXPECT_TRUE(request.valid());
  EXPECT_TRUE(request.interval_min == Model::kFastIntervalMin &&
              request.interval_max == Model::kFastIntervalFallbackMax);
  EXPECT_TRUE(request.max_ce_length == Model::kBulkMaxCeLength);
  EXPECT_TRUE(model.on_request_accepted(request, now));
  EXPECT_TRUE(model.on_interval_complete(request.handle, generation,
                                         Model::kStatusSuccess, 16U, now));
  EXPECT_TRUE(model.telemetry(0U).state == State::Ready);
  EXPECT_TRUE(model.telemetry(0U).confirmed_interval == 16U);

  /* Live profile unchanged: a non-bulk fast request keeps the small CE. */
  Model live_model;
  EXPECT_TRUE(live_model.connect(0U, 0x0041U, now));
  const uint32_t live_generation = live_model.telemetry(0U).generation;
  now += Model::kCommissioningStartDelayMs;
  request = live_model.issue_next(now);
  EXPECT_TRUE(request.valid() && request.procedure == Procedure::Dle);
  EXPECT_TRUE(live_model.on_request_accepted(request, now));
  EXPECT_TRUE(live_model.on_dle_complete(request.handle, live_generation, 251U, 251U, now));
  request = live_model.issue_next(now);
  EXPECT_TRUE(request.valid() && request.procedure == Procedure::Phy);
  EXPECT_TRUE(live_model.on_request_accepted(request, now));
  EXPECT_TRUE(live_model.on_phy_complete(request.handle, live_generation,
                                         Model::kStatusSuccess, 2U, 2U, now));
  EXPECT_TRUE(live_model.begin_fast_preparation(0U, live_generation,
                                                TuningWire::kDefaultFastInterval));
  request = live_model.issue_next(now);
  EXPECT_TRUE(request.valid());
  EXPECT_TRUE(request.interval_min == TuningWire::kDefaultFastInterval);
  EXPECT_TRUE(request.min_ce_length == Model::kLiveMinCeLength &&
              request.max_ce_length == Model::kLiveMaxCeLength);
}
...
int main()
{
  ...
  test_bulk_ce_hint_survives_fallback_ladder();
