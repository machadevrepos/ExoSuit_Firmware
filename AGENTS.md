...
- `firmware/master/`, `firmware/node/` — CubeIDE managed projects (`.ioc` at roots; lowercase directory names)
...
- No C++ host compiler on this PC (no g++, no zig, `gcc` has no working cc1plus): C++ module tests (incl. `host/tests/cpp`) must be compiled in the STM32CubeIDE environment. `cmake` exists but cannot build C++ targets here. `python3` works; `pytest` is not installed (plain `python3` assert scripts still run).
...
- GATT event masks: Master PipeDataTx 0x0F, Node PipeDataTx 0x0F (P1 applied 2026-09-13 on `feature/BLE_speed_Optimization`; completion counter is filtered to the PipeDataTx value handle in `custom_stm.cpp`), Node other chars 0x07. Length types stay `CHAR_VALUE_LEN_VARIABLE` on both sides (Master compiles its CONSTANT token to VARIABLE at `custom_stm.cpp:63-64`) — do not switch Node PipeDataTx to fixed-length: CPU2 behavior for shorter-than-declared updates is undocumented.
