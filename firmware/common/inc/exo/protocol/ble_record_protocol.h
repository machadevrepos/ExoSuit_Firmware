                                    ...
                                    uint8_t *dst, size_t dst_len)
{
    ...
    put_le32(&dst[2], message.session_id);
    for (uint8_t i = 0U; i < 8U; ++i) {
        dst[6U + i] = static_cast<uint8_t>(message.start_timestamp_us >> (8U * i));
    }
    put_le32(&dst[14], message.safety_duration_ms);
    put_le16(&dst[18], message.selected_source_mask);
                                    ...
                                    StartSessionV2Message &message)
{
    ...
    message.session_id = get_le32(&src[2]);
    message.start_timestamp_us = 0U;
    for (uint8_t i = 0U; i < 8U; ++i) {
        message.start_timestamp_us |= static_cast<uint64_t>(src[6U + i]) << (8U * i);
    }
    message.safety_duration_ms = get_le32(&src[14]);
    message.selected_source_mask = get_le16(&src[18]);
