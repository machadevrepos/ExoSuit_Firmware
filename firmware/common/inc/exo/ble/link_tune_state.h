...
class LinkTuneState {
 public:
  ...
  /* Full-window bulk connection-event hint (20 ms = 32 CE units of 0.625 ms).
   * The hint is informative to the CPU2 scheduler (AN5270) and cannot extend
   * an event past the interval, so it saturates the owned 20 ms bulk interval
   * (the former 30 ms hint only survived its own assert at the old 30 ms
   * interval, where 48 CE units <= 2*24 interval units held at equality). */
  static constexpr uint16_t kBulkMaxCeLength = 0x0020U; /* 20 ms */
   ...
   * Bulk parks the other links and saturates its own 20 ms interval with the
   * CE hint (kBulkMaxCeLength); the live preview instead gives every fast
   * leaf a SMALL connection-event budget so each can ship a few notifications
   * per event without starving its peers. Without this the WB scheduler
   * delivers ~1 packet per event per leaf (~31 samples/s), well under the
   * 50/s a node produces. Units: 0.625 ms. */
 ...
 private:
  ...
  static constexpr uint16_t max_ce_length(Interval interval,
    ...
    if (fast_interval == kBulkInterval) {
      /* Bulk profile: keep the generous connection-event hint at EVERY ladder
       * level. The CE length is an informative hint to the CPU2 scheduler
       * (AN5270); when the exact interval request is rejected and the ladder
       * demotes the profile to the live 5 ms hint, the granted interval drains
       * ~1 notification per event and the upload collapses to ~7-9 KB/s
       * (measured). A hint above the granted interval is legal - the
       * controller clamps the event to what fits. */
      (void)fallback_level;
