    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
		...
		case ACI_GATT_NOTIFICATION_COMPLETE_VSEVT_CODE:
        {
          ...
          /* Owned deviation (see the PipeDataTx creation note): the shared
           * completion counter drives the upload pump and live gate, so only
           * the one characteristic that actually carries the 0x08 mask
           * (PipeDataTx) may increment it. Any future 0x08 characteristic
           * would otherwise mis-wake both consumers. */
          if (notification_complete->Attr_Handle ==
              (CustomContext.CustomPipedatatxHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * fast path instead of the recovery watchdogs. The length type stays
   * VARIABLE on purpose: the Master compiles its own "CONSTANT" PipeDataTx to
   * VARIABLE (custom_stm.cpp:63-64 #undef/#define) and the fleet-proven 0x08
   * path runs on that configuration; every node frame (239 B chunk, smaller
   * RecordDone/live bundles) passes its real length to
   * aci_gatt_update_char_value, and CPU2 behavior for shorter-than-declared
   * updates on a fixed-length attribute is not documented. */
